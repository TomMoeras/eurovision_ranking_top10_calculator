        ...
        for pred_pos, country in enumerate(prediction):
            ...
            if in_top10:
                ...
                if difference == 0:
                    ...
                    explanation.append(f"+{points} points (off by {difference} position{'s' if difference > 1 else ''} in top 10)")
...
class TopHeavyPositionalProximity(ScoringSystem):
    ...
    def get_detailed_breakdown(self, prediction: List[str], actual_results: List[str]) -> Dict[str, Any]:
        ...
        # Print top 10 first
        for i, country in enumerate(top10_results):
            if i < 10:
        ...
        for pred_pos, country in enumerate(prediction):
            ...
            points = 0
            explanation = []
            tier = "not in top 10"  # Default tier value
...
class ExtendedPositionalProximity(ScoringSystem):
    """System 8: Extended Positional Proximity
    
    A scoring system that awards points based on proximity to actual position.
    
    This system is very similar to the Positional Proximity system but extends
    the scoring scale to be more granular. It awards points based on how close
    a predicted position is to the actual position, regardless of whether the 
    country finished in the top 10 or beyond.
    """
    
    def __init__(self):
        super().__init__(
            "Extended Positional Proximity",
...
class ModifiedTopHeavyProximity(ScoringSystem):
    """System 9: Modified Top-Heavy Proximity
    
    A less top-heavy version with base points for correctly predicted countries in top 10:
    ...
    def __init__(self, base_points: int = 2):
        super().__init__(
            "Modified Top-Heavy Proximity",
            f"+{base_points} base points per correct country + proximity points (less top-heavy)"
        )
